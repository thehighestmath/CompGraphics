...
SimpleObject3D *FigureBuilder::initCube(const QImage &texture, float width, float height, float depth, QVector3D center=QVector3D(0, 0, 0))
{
    ...
    vertexes.append(VertexData(QVector3D(center.x() - width_div_2, center.y() + depth_div_2, center.z() + height_div_2),
                               QVector2D(0.0, 1.0),
...
SimpleObject3D *FigureBuilder::initCylinder(QVector3D center1, QVector3D center2, double r1, double r2, double step)
{
    double zNormal;
    double delta = r2 - r1;
    if(delta > 0)
    ...
    QList<QVector3D> disk2Points;
    QList<QVector3D> disk2Normals;
    for(float i = 0; i <= 2 * M_PI + step; i += step)
    {
        disk2Points.push_back(QVector3D(disk2Center.position.x() + sin(i) * r2, disk2Center.position.y() + cos(i) * r2, disk2Center.position.z()));
        disk2Normals.push_back(QVector3D(sin(i), cos(i), -zNormal));
    }
